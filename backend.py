...
window.geometry("350x500+100+100")  #创建一个270x250+100+100，标题为“Your calculator“，在该程序中定义成window的窗口
...
recent_text = tk.Text(window, height=5, width=30, font=font2)
recent_text.grid(row=10, column=1, columnspan=4)

button_sin = tk.Button(window, text="Sin", width=5, font=font2, relief=tk.FLAT, bg="white")
button_cos = tk.Button(window, text="Cos", width=5, font=font2, relief=tk.FLAT, bg="white")
button_tan = tk.Button(window, text="Tan", width=5, font=font2, relief=tk.FLAT, bg="white")
button_sqrt = tk.Button(window, text="√", width=5, font=font2, relief=tk.FLAT, bg="white")
button_sin.grid(row=2, column=1, padx=3, pady=1)
button_cos.grid(row=2, column=2, padx=3, pady=2)
button_tan.grid(row=2, column=3, padx=3, pady=2)
button_sqrt.grid(row=2, column=4, padx=3, pady=2)
...
button_c.grid(row=3, column=1, padx=3, pady=1)
button_back.grid(row=3, column=2, padx=3, pady=2)
button_div.grid(row=3, column=3, padx=3, pady=2)
button_mul.grid(row=3, column=4, padx=3, pady=2)
...
button_7.grid(row=4,column=1,padx=3,pady=1)
button_8.grid(row=4,column=2,padx=3,pady=1)
button_9.grid(row=4,column=3,padx=3,pady=1)
button_min.grid(row=4,column=4,padx=3,pady=1)
...
button_4.grid(row=5,column=1,padx=3,pady=1)
button_5.grid(row=5,column=2,padx=3,pady=1)
button_6.grid(row=5,column=3,padx=3,pady=1)
button_add.grid(row=5,column=4,padx=3,pady=1)
...
button_1.grid(row=6,column=1,padx=3,pady=1)
button_2.grid(row=6,column=2,padx=3,pady=1)
button_3.grid(row=6,column=3,padx=3,pady=1)
button_equ.grid(row=6,column=4,padx=3,pady=1,rowspan=2)
...
button_0.grid(row=7,column=1,padx=3,pady=1,columnspan=2)
button_dot.grid(row=7,column=3,padx=3,pady=1)
...
def calculation():
    ...
    opt_str = res.get()
    tmp = eval(opt_str)
    res.set("{:.3e}".format(tmp))
    operation = str(opt_str) + "=" + str(tmp)
...
def update_recent_operations():
    ...
    for operation in recent_operations:
        recent_text.insert(tk.END, operation + "\n")
...
def load_operations():
    try:
        with open(file_path, "r") as file:
            ...
            recent_operations = [line.strip() for line in lines]
            update_recent_operations()
...
button_tan.config(command=lambda: tan())
button_cos.config(command=lambda: cos())
button_sin.config(command=lambda: sin())
button_sqrt.config(command=lambda: sqrt())
